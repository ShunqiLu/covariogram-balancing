...
\pacs[MSC Classification]{52C07, 11H06}
...
\begin{figure}[ht]
  ...
  \caption{Logical architecture.  Centered exchange fibers produce atomic
  $\{0,1\}$ exposures, whose sum gives the fixed-shell order and a fiberwise
  equality certificate.  The quantitative layer is body-dependent; the
  structural theorem of Section~\ref{sec:structural} evaluates it for
  $C_d+bB_\infty^d$ and recovers the Lee and the cube regime as the cases
  without factorization and without surcharge.}
...
\caption{The exchange fiber of Definition~\ref{def:exchange-fiber} in a
symmetric planar section.  Fixing the residual coordinates and the sum
$x_i+x_j=r$ cuts out a segment of the body; its lattice points, recorded
...
\caption{The three regimes of Lemma~\ref{lem:atomic-exposure}.  Moving the
parity interval $I_\beta-D$ two steps toward $I_\alpha$ removes its left
endpoint and adds a new right endpoint, so the overlap can grow by at most
one point.  It grows by exactly one when the new endpoint enters
$I_\alpha$ while the old one was already outside~(a); it is unchanged when
$I_\alpha$ contains both~(b) or neither~(c).}
