...
\caption{The three regimes of Lemma~\ref{lem:atomic-exposure}, each with its
own $(\alpha,\beta,D)$.  In every panel the middle row is $I_\alpha$, the row
above it is $I_\beta-D$, and the row below is that set translated by two;
dashed links mark the points each overlap counts.  The translation deletes the
endpoint $-\beta-D$ and appends $\beta-D+2$, the two ringed markers, so the
count changes by at most one.  It rises when the appended endpoint lies in the
window $|x|\le\alpha$ while the deleted one did not~(a), and is unchanged when
both lie inside~(b) or both outside~(c); the fourth combination cannot occur
because $|\beta-D+2|\le\beta+D$.  The bottom line is the partition of the
$\alpha$-axis at fixed $\beta$ and $D$ that these three regimes produce, which
is the criterion~\eqref{eq:atomic-exposure-criterion}.}
