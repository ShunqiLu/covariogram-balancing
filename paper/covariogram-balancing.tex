...
% Float placement: allow a slightly larger bottom float so that a figure can
% stay on the page of the statement it illustrates.
...
study how this cross-covariogram varies as the coordinates of $u$ are
balanced along a fixed $\ell_1$ shell.  A centered exchange-fiber property,
satisfied by lattice sections of compact convex sets invariant under signed
coordinate permutations, decomposes every unit balancing transfer into
parity-fiber endpoint exposures in $\{0,1\}$, yielding the Schur order on each
shell and a fiberwise equality certificate for comparable shifts.  For equal
Lee balls of radius $t$ in dimension $d\ge3$ and comparable shifts on an
active shell $2\le s\le2t$, the overlap deficit is at least a closed-form
constant times the number of unit transfers; a central-box section theorem
evaluates it in lower-dimensional Lee-ball counts.  Incomparable shifts may
share a value, so equality is order-theoretic.  The extremizers
are characterized for $4\le s\le2t$: each interior shell carries exactly
...
shells form a boundary family.  A structural theorem evaluates the sharp
layer in every dimension for the Minkowski interpolants $C_d+bB_\infty^d$
between cross-polytope and cube: the constant is determined by lattice-point
counts of the same body two dimensions down, plus an explicit facet surcharge
for transfers into a zero coordinate, and one inequality decides the
minimizing transfer.  Cubes and Lee balls are its degenerate regimes.  Convex
hyperoctahedral symmetry thus fixes the transfer sign but neither the sharp
scale nor the minimizing orbit; reversals occur once convexity or
coordinate-permutation symmetry fails.}
...
\caption{The three regimes of Lemma~\ref{lem:atomic-exposure}.  In
(a)--(c) the middle row is $I_\alpha$, the row above it is $I_\beta-D$, and
the row below is that set translated by two; the shaded strip is the window
$|x|\le\alpha$, open markers lie outside it, and dashed links mark the
coincidences, counted on the right.  The translation deletes the endpoint
$-\beta-D$ and appends $\beta-D+2$, the two ringed markers, so the count
changes by at most one: it rises when the appended endpoint lies in the window
while the deleted one did not~(a), and is unchanged when both lie inside~(b)
or both outside~(c).  The remaining combination cannot occur because
$|\beta-D+2|\le\beta+D$.  Panel~(d) is the partition of the $\alpha$-axis at
fixed $\beta$ and $D$ that the three regimes produce, which is the
...
\caption{A unit balancing transfer and what it does to the lens.  Both shifts
lie on the shell $\|u\|_1=2$ and differ by one unit moved from the larger
coordinate to the smaller one.  In each panel the blue diamond is $B_1^2(2)$,
the orange one is $B_1^2(2)-u$, and the filled points are the lattice points
they share; the count rises from five to eight.}
\label{fig:transfer}
\end{figure}
...
scalar Lee distance does not determine the overlap.  The two shifts are
drawn in Figure~\ref{fig:transfer}.
...
\]
The elementary move underlying the order sends one unit from the larger
coordinate to the smaller one while their sum stays fixed; it is the move
relating the two shifts of Figure~\ref{fig:transfer}.
...
\caption{The diamond-to-square map.  $T(x,y)=(x+y,x-y)$ is a bijection of
$\Z^2$ onto $\Lambda=\{(r,w)\in\Z^2:r\equiv w\pmod2\}$, not onto $\Z^2$: the
crosses in (b) are the integer points it misses.  Discs and squares mark the
channels $r$ even and $r$ odd, which the map keeps separate.}
...
\caption{The incidence bound of Lemma~\ref{lem:face-incidence} for $h=2$ and
$w=(3,1)$, so $c=4$ and $a=2$.  A facet misses $\Lambda_2$ exactly when the
coordinate it freezes is too heavy, here $q_1=0$ and $q_1=3$; the other two
meet it, matching $2^1\binom{h-1}1=2$.  Neither vertex meets the layer,
matching $2^2\binom{h-1}2=0$.}
...
constant drops steeply off the two innermost shells.  The labelled markers
are the last active shells; beyond them both constants vanish.  In $d=3$ the
two branches of~\eqref{eq:kappa-closed} agree on the common value $2t-s+1$
for $s\ge4$, so the even--odd split is visible only from $d\ge4$ on.
Panel~(b) plots the
...
candidate arcs of~\eqref{eq:structural-two-candidates} as donor, recipient,
and residual coordinates, with bar heights schematic: the shaded unit is the
one transferred and the dashed outline is where it lands.  The
recipient-zero arc buys a larger residual and pays the facet surcharge.  In
(b) the threshold~\eqref{eq:threshold} at every integer dimension for three
pairs $(b,s)$: it decreases with slope $-2$, so each pair has a single zero,
at which both arcs are minimizing.}
\label{fig:threshold}
\end{figure}
...
balancing move decomposes exactly into parity-fiber endpoint exposures, each
zero or one; convex hyperoctahedral lattice sections have such fibers.  This
gives a fiberwise equality certificate along the fixed-shell transfer graph
and, for equal Lee balls with $d\ge3$, the closed sharp constant on every
active shell, all minimizing arcs for $4\le s\le2t$, and distance-one rigidity
of the extremizers on interior shells.  Cubes factor into interval overlaps,
with a product-form constant.  Section~\ref{sec:structural} places all three
behaviours in one mechanism: for $C_d+bB_\infty^d$ an arc with positive
recipient weighs its gap minus one times the self-overlap of the same body two
dimensions down, while an arc into a zero coordinate pays a surcharge from the
two capped layers of the cube summand.  The sharp constant is therefore
determined by lower-dimensional lattice-point counts with an explicit facet
surcharge, and the sign of $\Theta_{d,b}(s)$ decides which arc realizes it.
Cubes have no surcharge and Lee balls no factorization, which is why their
sharp layers have one and $\lfloor s/2\rfloor-1$ minimizing arcs: the sign is a
matter of symmetry, the sharp scale and the extremizing geometry are not.
